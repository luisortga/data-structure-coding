...
const cusomerActor = {
    ...
    'put on table' : (msg, ctx, _state) => {
        console.log(`${ctx.self.name} sees "${msg.food}" aparecer en la mesa`)
    },
    'no pie left': (_msg, ctx, _state) => 
        console.log(`${ctx.self.name} sulk...`)
}
    ...
    "order": (msg, ctx, state) => {
        if (msg.wants === "pie") {
            dispatch(state.pieCase, {
                type: "get slice", costumer: msg.customer, waiter: ctx.self })
        } else  {
            console.dir(`Dont know to order ${msg.wants}`)
        }
    },
...
const pieCaseActor = {
    'get slice': (msg, context, state) => {
        if (state.slice.length === 0) {
            ...
            let slice = state.slices.shift() + ' pie slice'
...
let pieCase = start_actor(
    actorSystem,
    'pie-case',
    pieCaseActor,   {   slices: ["apple", "peach", "cherry"] })
